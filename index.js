    ...
    for (let param of params) {
      ...
      if (typeof parsedValue === "object") {
        ...
        for (var key in parsedValue) {
          key = stripPrefix ? key.replace(`${stripPrefix}/`, "") : key
        ...
        var envVarName = stripPrefix ? split[split.length - 1].replace(`${stripPrefix}-`, "") : split[split.length - 1]
        core.debug(`Using prefix + end of ssmPath for env var name: ${envVarName}`)
        setEnvironmentVar(prefix + envVarName, parsedValue, maskValues)
      }
    }
  } catch (e) {
